...
const LessonNav = ({ navData,firstLessonId, onEdit,setOpen,open }) => {

const deleteLesson=async()=>{
try {
    if(!window.confirm("Are you sure to delete this lesson")) return;
    const res= await axios.delete(`http://localhost:5000/instructor/lesson/${navData.lessonId}`,{withCredentials:true});
    if(res.data?.success){
     window.location.href=`/instructor/course/${navData.courseId}/lesson/${firstLessonId}`;
    }
  } catch (error) {
    console.log(error)
  }
...
const {courseId}=useParams()
 return (
    ...
    {navData.lessonId && (
       <Link to={`/instructor/lesson/${courseId}`} state={navData.lessonId}> <button
          onClick={onEdit}
