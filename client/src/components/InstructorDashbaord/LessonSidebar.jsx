...
const LessonSidebar = ({  open,setOpen, setFirstLessonId}) => {
...
const handleLesson = async () => {
  try {
    const res = await axios.get(
      `http://localhost:5000/instructor/lessons/${courseId}`,
      { withCredentials: true }
    );
   
   setLessons(res.data?.data || [])
  setFirstLessonId(res.data?.data?.[0]?._id)
  } catch (error) {
    console.log(error);
  }
};
      ...
      >
        ...
        <div className="p-5 border-b flex  justify-between gap-9  items-center">
          ...
          {/* Close Button (mobile only) */}
           ...
           <Link to={`/instructor/lesson/${courseId}`} className="w-[70%] m-auto  text-center bg-black text-white py-2 rounded-lg">
            Add Lesson
         </Link>
           <button
            onClick={() => setOpen(false)}
