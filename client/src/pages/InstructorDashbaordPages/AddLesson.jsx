...
const handleLesson = async () => {
  ...
  const res = await axios.get(
          `http://localhost:5000/instructor/lessons/${courseId}/${state}`,
...
const handleSubmit = async(e) => {
   ...
   try {
    ...
    const  res= await axios.post(`http://localhost:5000/instructor/lesson/create/${courseId}`,data,{withCredentials:true});
   ...
   if (res.data?.success){
    ...
    setLoading(false);
    nav(`/instructor/course/${courseId}/lesson/${res.data?.data?._id}`);
   }
   } catch (error) {
    console.log(error)
   }
...
const updateSubmit = async(e) => {
   ...
   try {
    ...
    const  res= await axios.put(`http://localhost:5000/instructor/lesson/${state}`,data,{withCredentials:true});
   if (res.data?.success){
    setAction({
      show:true,
      message:res.data?.message,
