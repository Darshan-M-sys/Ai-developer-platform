...
const AddCourses = () => {
  ...
  const {state}=useLocation();
   ...
   const handleGetCourseData=async()=>{
      try {
        if(state){
        const res= await axios.get(`http://localhost:5000/instructor/course/${state}`,{withCredentials:true});
         setFormData((prev)=>({...prev,...res.data?.courseData}))
         setIsUpdate(true)
        }
      } catch (error) {
        console.log(error)
 ...
 const handleUpdateCourse=async(e)=>{
    ...
    try {
      ...
      const res= await axios.put(`http://localhost:5000/instructor/course/${state}`,data,{withCredentials:true});
       if(res.data?.success){
        setAction({
          show:true,
...
setFormData((prev)=>({...prev,[e.target.name]:e.target.value}))
  ...
  useEffect(()=>{
   ...
   const handleCreateCourse=async(e)=>{
    ...
    try {
      ...
      const res= await axios.post("http://localhost:5000/instructor/course/create",data,{withCredentials:true});
       if(res.data?.success){
        setAction({
          show:true,
          message:res.data?.message,
  ...
  return (
    ...
    <Header/>
    <div className="md:ml-[280px] md:mt-[66px] mt-[55px]">
  <Sidebar/>
    </div>
